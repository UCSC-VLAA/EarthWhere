...
def shapley_reweight(thinking_results,bvid):
    gt_weight_path = '/openrlhf/Geolocation/bili/eval/gt_key_clues_weight.json'

    with open(gt_weight_path, 'r', encoding='utf-8') as f:
        gt_weight = json.load(f)


    score = sum([
        a * b for a, b in zip(thinking_results, gt_weight[bvid])
    ])
        
    return score

def thinking_score(bvid:str,thinking_process: str, key_clues: str, gemini_model) -> float:
    ...
    score  = shapley_reweight(result,bvid)
...
def evaluate_prediction(
    ...
    think_score, result = thinking_score(bvid,thinking_process, key_clues, gemini_model)
...
def run_evaluation(args: argparse.Namespace) -> None:
    ...
    if os.path.exists(eval_path):
        ...
        for bvid, content in existing_data.items():
            result = {
                ...
                'predicted': content['predicted'],
                'response': content['response'],
                'answer_score': float(content['answer_score']),
                'thinking_score': float(content['thinking_score']),
    ...
    bili_coor_count = 0
    yt_coor_count = 0
    bili_coor_distance_list = []
    bili_coor_thinking = 0.0
    yt_coor_distance_list = []
    yt_coor_thinking = 0.0

    bili_text_count = 0
    yt_text_count = 0
    bili_text_answer_score = 0.0
    bili_text_thinking = 0.0
    yt_text_answer_score = 0.0
    yt_text_thinking = 0.0
    ...
    for result in existing_eval_results:
        ...
        total_predictions += 1
        # if result['BVID'].startswith('BV'):
        #     bili_count += 1
        #     bili_answer_score += float(result['answer_score'])
            
        # else:
        #     yt_count += 1
        #     yt_answer_score += float(result['answer_score'])

        if result['BVID'].startswith('BV'):
            if result['answer_type'] == 'latitude & longitude':
                bili_coor_count += 1
                bili_coor_distance_list.append(result['distance'])
                bili_coor_thinking += float(result['thinking_score'])
            else:
                bili_text_count += 1
                bili_text_answer_score += float(result['answer_score'])
                bili_text_thinking += float(result['thinking_score'])
        else:
            if result['answer_type'] == 'latitude & longitude':
                yt_coor_count += 1
                yt_coor_distance_list.append(result['distance'])
                yt_coor_thinking += float(result['thinking_score'])
            else:
                yt_text_count += 1
                yt_text_answer_score += float(result['answer_score'])
                yt_text_thinking += float(result['thinking_score'])
    ...
    for _, row in tqdm(df.iterrows()):
        ...
        if str(row['BVID']).startswith('BV'):
            if row['answer_type'] == 'latitude & longitude':
                bili_coor_count += 1
                bili_coor_distance_list.append(evaluation_result['distance'])
                bili_coor_thinking += float(evaluation_result['thinking_score'])
            else:
                bili_text_count += 1
                bili_text_answer_score += float(evaluation_result['answer_score'])
                bili_text_thinking += float(evaluation_result['thinking_score'])
        else:
            if row['answer_type'] == 'latitude & longitude':
                yt_coor_count += 1
                yt_coor_distance_list.append(evaluation_result['distance'])
                yt_coor_thinking += float(evaluation_result['thinking_score'])
            else:
                yt_text_count += 1
                yt_text_answer_score += float(evaluation_result['answer_score'])
                yt_text_thinking += float(evaluation_result['thinking_score'])


        eval_results.append(new_result)
        processed_bvids.add(row['BVID'])
        print(f"BVID: {row['BVID']}, predicted: {row['predicted']}, ground_truth: {row['ground_truth']}")
    ...
    # bili_coor_accuracy = bili_coor_answer_score / bili_coor_count if bili_coor_count > 0 else 0.0
    # yt_coor_accuracy = yt_coor_answer_score / yt_coor_count if yt_coor_count > 0 else 0.0
    #bili_acc
    bili_acc1 = calculate_distance_acc(bili_coor_distance_list,1)
    bili_acc5 = calculate_distance_acc(bili_coor_distance_list,5)
    bili_acc10 = calculate_distance_acc(bili_coor_distance_list,10)
    bili_acc20 = calculate_distance_acc(bili_coor_distance_list,20)
    bili_acc50 = calculate_distance_acc(bili_coor_distance_list,50)
    bili_acc100 = calculate_distance_acc(bili_coor_distance_list,100)
    bili_acc200 = calculate_distance_acc(bili_coor_distance_list,200)

    #yt_acc
    yt_acc1 = calculate_distance_acc(yt_coor_distance_list,1)
    yt_acc5 = calculate_distance_acc(yt_coor_distance_list,5)
    yt_acc10 = calculate_distance_acc(yt_coor_distance_list,10)
    yt_acc20 = calculate_distance_acc(yt_coor_distance_list,20)
    yt_acc50 = calculate_distance_acc(yt_coor_distance_list,50)
    yt_acc100 = calculate_distance_acc(yt_coor_distance_list,100)
    yt_acc200 = calculate_distance_acc(yt_coor_distance_list,200)

    bili_text_accuracy = bili_text_answer_score / bili_text_count if bili_text_count > 0 else 0.0
    yt_text_accuracy = yt_text_answer_score / yt_text_count if yt_text_count > 0 else 0.0
    wherecountry_count = 500 if wherecountry_accuracy != 0.0 else 0
    overall = (wherecountry_count*wherecountry_accuracy+ bili_coor_count*bili_acc1 + yt_coor_count*yt_acc1+bili_text_count*bili_text_accuracy+yt_text_count*yt_text_accuracy)/(wherecountry_count+bili_coor_count+bili_text_count+yt_coor_count+yt_text_count)
    ...
    if model_basename in overall_df['model_name'].values:
        # Check if wherestreet results already exist for this model
        existing_row = overall_df[overall_df['model_name'] == model_basename].iloc[0]
        has_wherestreet_results = (
            'wherestreet-bilibili_Count' in existing_row and 
            pd.notna(existing_row.get('wherestreet-bilibili_Count')) and
            existing_row.get('wherestreet-bilibili_Count') > 0
        )
        
        if has_wherestreet_results:
            # Only update wherecountry if wherestreet results already exist
            print(f"Wherestreet results already exist for {model_basename}, only updating wherecountry")
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherecountry_Accuracy'] = wherecountry_accuracy
        else:
            # Update all values including wherestreet
            print(f"Updating all results for {model_basename}")
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherecountry_Accuracy'] = wherecountry_accuracy
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili_Count'] = bili_text_count
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili_Accuracy'] = bili_text_accuracy
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili_Thinking'] = bili_text_thinking
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT_Count'] = yt_text_count
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT_Accuracy'] = yt_text_accuracy
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT_Thinking'] = yt_text_thinking
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Count'] = bili_coor_count
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@1km'] = bili_acc1
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@5km'] = bili_acc5
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@10km'] = bili_acc10
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@20km'] = bili_acc20
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@50km'] = bili_acc50
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@100km'] = bili_acc100
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Acc@200km'] = bili_acc200
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-bilibili-coord_Thinking'] = bili_coor_thinking
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Count'] = yt_coor_count
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@1km'] = yt_acc1
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@5km'] = yt_acc5
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@10km'] = yt_acc10
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@20km'] = yt_acc20
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@50km'] = yt_acc50
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@100km'] = yt_acc100
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Acc@200km'] = yt_acc200
            overall_df.loc[overall_df['model_name'] == model_basename, 'wherestreet-YT-coord_Thinking'] = yt_coor_thinking
            overall_df.loc[overall_df['model_name'] == model_basename, 'overall_Accuracy'] = overall
    else:
        new_row = pd.DataFrame({
            'model_name': [model_basename],
            'wherecountry_Accuracy': [wherecountry_accuracy],
            #wherestreet text
            'wherestreet-bilibili_Count': [bili_text_count],
            'wherestreet-bilibili_Accuracy': [bili_text_accuracy],
            'wherestreet-bilibili_Thinking': [bili_text_thinking],
            'wherestreet-YT_Count': [yt_text_count],
            'wherestreet-YT_Accuracy': [yt_text_accuracy],
            'wherestreet-YT_Thinking': [yt_text_thinking],
            #wherestreet coord
            'wherestreet-bilibili-coord_Count': [bili_coor_count],
            'wherestreet-bilibili-coord_Acc@1km': [bili_acc1],
            'wherestreet-bilibili-coord_Acc@5km': [bili_acc5],
            'wherestreet-bilibili-coord_Acc@10km': [bili_acc10],
            'wherestreet-bilibili-coord_Acc@20km': [bili_acc20],
            'wherestreet-bilibili-coord_Acc@50km': [bili_acc50],
            'wherestreet-bilibili-coord_Acc@100km': [bili_acc100],
            'wherestreet-bilibili-coord_Acc@200km': [bili_acc200],
            'wherestreet-bilibili-coord_Thinking': [bili_coor_thinking],
            'wherestreet-YT-coord_Count': [yt_coor_count],
            'wherestreet-YT-coord_Acc@1km': [yt_acc1],
            'wherestreet-YT-coord_Acc@5km': [yt_acc5],
            'wherestreet-YT-coord_Acc@10km': [yt_acc10],
            'wherestreet-YT-coord_Acc@20km': [yt_acc20],
            'wherestreet-YT-coord_Acc@50km': [yt_acc50],
            'wherestreet-YT-coord_Acc@100km': [yt_acc100],
            'wherestreet-YT-coord_Acc@200km': [yt_acc200],
            'wherestreet-YT-coord_Thinking': [yt_coor_thinking],
